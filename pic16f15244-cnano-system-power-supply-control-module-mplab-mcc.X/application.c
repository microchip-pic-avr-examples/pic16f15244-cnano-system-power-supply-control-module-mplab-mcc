 ...
 * Author: I51005
 *
 * Created on February 1, 2024, 9:18 AM
 */
...
void I2CAndSleepHandler(void)
{
    ...
    {
        i2cStopFlag = FALSE;
        RegulatorControl(ON);    //Turn ON the regulator before entering into waiting period 30 secs
         __delay_ms(1000);       //Delay is added to showcase the regulator is ON
    ...
    if(Flags.i2cNewSleepCommand)
    {
        ...
        if(timerFlag == 1)
        {
            ...
            timerFlag = CLEAR;    //Clear the timerFlag
             //wait for 30 seconds before turning of regulator
            Flags.regulatorOff = WaitForTime1(WAIT_PERIOD_30_SECS);  //30 seconds
        }
        else
        {
    ...
    if(Flags.regulatorOff)
    {
        ...
        RegulatorControl(OFF);  //Turn off the regulator
        printf("Regulator Off");  //for debug use
         __delay_ms(50);
    ...
    if(Flags.continueSleepMode)
    {
        ...
        if(NoOfSleepCounts >= (regulatorOffDuration/WATCHDOG_TIME_DURATION)) // (regulatorOffDuration/WATCHDOG_TIME_DURATION) = (32/8) = 4
        {
            ...
            RegulatorControl(ON);   //Turn on the regulator
             __delay_ms(1000);
            printf("Regulator On");   //for debug use
             __delay_ms(50);
...
bool ProcessI2CCommand(uint8_t *dataBuf, uint8_t len)
{
    ...
    else									//Valid command
    {
        ...
        printf("T %d\n", regulatorOffDuration);   //for debug use
         __delay_ms(50);
        return TRUE;
    }
    return FALSE;
